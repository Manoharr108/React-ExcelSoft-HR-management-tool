...
const { setProgress, clearProgress } = require("./emailprogress");
...
const getCongratulatoryEmailTemplate = (employeeName, quarter, category, hrMessage) => {
  return {
    ...
    html: `
      ...
      <head>
        ...
        <style>
          @media only screen and (max-width: 600px) {
            ...
            .header { padding: 15px !important; font-size: 24px !important; }
            .body-content { padding: 20px !important; }
      ...
      <body>
        <div class="container" style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f6f8; padding: 20px; max-width: 700px; margin: 0 auto; border-radius: 8px; border: 1px solid #e0e0e0;">
          <div class="header" style="background-color: #4CAF50; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
            <h1 style="margin: 0; font-size: 32px;">🎉 Congratulations!</h1>
            <p style="margin: 5px 0 0 0; font-size: 18px;">You’ve achieved excellence!</p>
          </div>

          <div class="body-content" style="padding: 30px; background-color: #ffffff; border-radius: 0 0 8px 8px;">
...
// 4. Mass email sending with progress tracking
const sendMassEmails = async (employees, quarter, hrMessage) => {
  const results = [];
  const total = employees.length;

  // console.log(`📨 Starting email blast to ${total} employees for Quarter ${quarter}`);
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (employee.mail && employee.name) {
      ...
      setProgress(quarter, percent);

      await new Promise((resolve) => setTimeout(resolve, 3000)); // delay
    } else {
      // console.warn(`⚠️ Skipping ${employee.empid}: missing email or name`);
      results.push({
        empid: employee.empid,
