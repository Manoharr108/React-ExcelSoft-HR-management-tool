...
const { getProgress } = require("./emailprogress");

exports.AddEmplyoee = async (req, res) => {
  let { empid, name, category, quarter, mail, remarks, role, epublic } =
    req.body;
  try {
    const duplicateEntry = await aoperation.findOne({
      empid,
      category,
      quarter,
    });
    if (duplicateEntry) {
      return res
        ...
        .json({
          message:
            "Employee already exists in the achievers list for this category and quarter.",
        });
    }
    ...
    return res
      .status(201)
      .json({ message: "Employee added to achievers list", newEmp });
  } catch (err) {
...
exports.AddTab = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Tab added to achivevers list", newtab });
  } catch (err) {
    return res.status(500).json({ message: err.message });
  }
};
...
exports.DeleteEmplyoee = async (req, res) => {
  ...
  try {
    ...
    if (emp) {
      return res
        .status(200)
        .json({ message: "Successfully deleted from the particlular tab!!" });
    } else {
      return res.status(404).json({ message: "Employee not found" });
    }
  } catch (error) {
    ...
    let emp = await aoperation
      ...
      .then(() => {
        return res
          .status(200)
          .json({
            message: "Successfully removed the whole " + category + " tab",
          });
      });
  } catch (error) {
    return res.status(500).json({ message: error.message });
...
exports.publishquarter = async (req, res) => {
  try {
    ...
    const publishedEmployees = await aoperation
      .find({
        quarter: activeQuarter,
        epublic: true,
        name: { $exists: true, $ne: "" },
    ...
    setTimeout(async () => {
      try {
        console.log(
          `Starting email notification process for ${publishedEmployees.length} employees`
        );
        const emailResults = await sendMassEmails(
          publishedEmployees,
          activeQuarter,
          hrMessage
        );
...
exports.AllEmployeeWithQuarter = async (req, res) => {
  ...
  try {
    ...
    if (!employees || employees.length === 0) {
      return res
        .status(404)
        .json({ message: "No employees found for this quarter." });
    }
